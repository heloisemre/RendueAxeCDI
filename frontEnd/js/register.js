...
const password2 = document.getElementById('password-confirm'); // get the password-confirm input field
...
form.addEventListener('submit', function(event) { // when the form is submitted
    ...
    fetch('http://localhost:3000/users', { // send a POST request to the server
        method: 'POST', 
        headers: {
    ...
    .then(data => { 
        if (data.message) { // if there is an error message, display it
            alert(data.message);
        } else {
